	...
	flecs::entity entity, std::vector<uint64_t> &entities)
{
	...
	if (trans == nullptr) {
		LOG_ERROR("No ComponentStaticTransform found for trigger entity %lu",
				  entity.id());
		return;
	}
	...
	if (shape == nullptr) {
		LOG_ERROR("No ComponentCollisionShape found for trigger entity %lu",
				  entity.id());
		return;
	}
	...
	cb.callback = (CbT) + [](Cb *cb, uint32_t eid) {
		...
		if (auto *t = e.try_get<ComponentMovementState>()) {
			...
			float moveFactor = 0;
			glm::vec3 normal;
			if (cb->triggerShape->shape.RayTest(cb->triggerTrans->trans, ray,
												moveFactor, normal)) {
