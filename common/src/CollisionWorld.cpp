								 ...
								 const ComponentShape &shape, glm::vec3 pos)
{
	...
	object->setWorldTransform(btTransform(btQuaternion::getIdentity(), ToBullet(pos)));
...
{
	pos.y += shape.height * 0.5f;
	obj.object->setWorldTransform(btTransform(btQuaternion::getIdentity(), ToBullet(pos)));
